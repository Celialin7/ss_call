...
class AdvancedCallCoverageChecker:
    ...
    def parse_script_variations(self, script_text):
        ...
        # Split by common separators: periods, semicolons, newlines, numbered lists
        # Also handle version indicators like "Version A:", "Version B:", etc.
        variations = []
        
        # First, split by major separators
        raw_splits = re.split(r'[.;\n]|版本[AB][:：]|Version [AB][:：]|情況[一二三四五六七八九十][:：]', script_text)
        
        for split in raw_splits:
            split = split.strip()
            if split and len(split) > 5:  # Filter out very short fragments
                # Further split by commas if the segment is very long
                if len(split) > 50:
        ...
        # If no variations found, return the original text as single variation
        if not unique_variations:
            unique_variations = [script_text]
...
# ==========================================================
def run_analysis(call_file_path, script_file_path, script_sheet_name, output_folder):
    """
    ...
    # 🔍 通话类型检测逻辑
    # 获取文件名并转换为小写进行比较
    filename = os.path.basename(call_file_path).lower()
    if "sqccb" in filename:
        call_type = "SQCCB"
    else:
        call_type = "Sales Call"
    
    print(f"🔍 检测到的通话类型: {call_type}")
    ...
    # Load call-specific weights and validate language support
    if call_type == "Sales Call":
        # Sales Call 模式：加载特定的权重和配置
        checker.load_call_specific_weights(call_file_path, script_df, script_sheet_name)
    else:
        # SQCCB 模式：跳过特定权重加载，使用默认配置
        print(f"🔍 SQCCB 模式：跳过特定权重加载，使用默认配置")
        # 设置默认语言和权重
        checker.current_language = "CAN"  # 默认粤语
        import sys
        sys.path.insert(0, '.')
        import dictionaries
        checker.stopwords = dictionaries.cantonese_stopwords
        checker.current_weights = getattr(dictionaries, 'term_importance', {})
    
    # Check if the detected language is supported
    if checker.current_language is None:
        print(f"❌ Error: Unable to detect language from filename.")
        print(f"Cantonese version only supports _C (Cantonese) and _E (English) files.")
        print(f"For Mandarin files (_M), please use the Mandarin version.")
        return {'status': 'ERROR', 'error': 'Language detection failed', 'output_file': None, 'coverage_rate': 0.0}
    elif checker.current_language not in ['CAN', 'ENG']:
        print(f"❌ Error: Detected language '{checker.current_language}' is not supported in Cantonese version.")
        print(f"Cantonese version only supports _C (Cantonese) and _E (English) files.")
    ...
    # STEP 1: EARLY SEPARATION - Detect system recordings first
    step3_start = time.time()
    print("\n📍 步骤1: 早期分离 - 检测系统录音...")
    system_recording_flags = checker.detect_system_audio_recordings(call_df)
    
    # Split call_df into two separate DataFrames
    analysis_df = call_df[~system_recording_flags].copy()  # Human dialogue only
    system_audio_df = call_df[system_recording_flags].copy()  # System recordings only
    
    # Create updated_call_df with System_Audio_Recording flags
    updated_call_df = call_df.copy()
    updated_call_df['System_Audio_Recording'] = system_recording_flags.map({True: 'YES', False: 'NO'})
    
    print(f"✅ 早期分离完成:")
    print(f"   - 人类对话行数: {len(analysis_df):,}")
    print(f"   - 系统录音行数: {len(system_audio_df):,}")
    print(f"   - 原始总行数: {len(call_df):,}")
    
    # STEP 2: INDEPENDENT PROCESSING - Only process human dialogue
    step4_start = time.time()
    print("\n📍 步骤2: 独立处理 - 仅对人类对话进行分组...")
    
    # Build grouped lines ONLY from human dialogue (analysis_df)
    grouped_lines, original_to_group = checker.build_grouped_lines_for_analysis(analysis_df)
    
    print(f"✅ 人类对话处理完成:")
    print(f"   - 从 {len(analysis_df)} 人类对话行创建了 {len(grouped_lines)} 个组")
    print_checkpoint(4, f"创建并保存分组通话数据 ({len(grouped_lines)} 个分组)", step4_start)
    ...
    results = checker.check_coverage(updated_call_df, script_df, grouped_lines, threshold=0.3)
