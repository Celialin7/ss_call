...
# Import configuration from config.py (fail-fast to avoid silent wrong profile)
from config import (
    SCRIPT_FILE_PATH,
    SBERT_MODEL_PATH,
...
def apply_composite_point_key_if_configured(script_df, sheet_name):
    ...
    if note_col is None:
        print(f"⚠️  {sheet_name}: Missing '{COMPOSITE_POINT_NOTE_COLUMN}', skip composite point key")
        return script_df
    ...
    print(f"🧩 {sheet_name}: Composite point key enabled ({required_col} + {note_col})")
    return updated_df

def preprocess_text(text, language='CAN'):
    """Enhanced text preprocessing with language-specific handling.
    
    Args:
        text: Input text to preprocess
        language: 'CAN'/'MAN' for Chinese (removes English), 'ENG' for English (keeps English)
    """
    if language == 'ENG':
        # English: Keep English letters, basic normalization
        import unicodedata
        ...
        # Chinese: Keep Chinese characters (simplified/traditional) and specified punctuation
...
def tokenize_text(text, language='CAN', remove_stopwords=True):
    ...
    if language == 'CAN':
        ...
        # English: Use english_resource for proper English tokenization
        try:
            import english_resource
            # 使用english_resource但不依赖其内部停用词过滤，统一用dictionaries
            import re
            import unicodedata
            # 基础英文预处理和分词
            normalized = unicodedata.normalize('NFKC', str(text))
            tokens = re.findall(r'\$[\d,]+\.?\d*|[\d,]+\.?\d*%|\d+[kKxX]|\w+', normalized.lower())
        except ImportError:
            # Fallback: Basic English tokenization
            import re
            tokens = re.findall(r'\w+', text.lower())
    elif language == 'MAN':
        # Mandarin: Use jieba segmentation
...
def generate_script_embeddings(all_scripts_df, model, output_path):
    ...
    Returns:
        dict: Dictionary mapping preprocessed script sentences to their embeddings
    """
    ...
    # Parse all scripts/variations and collect unique preprocessed sentences
    ...
    # Final embeddings dictionary uses preprocessed text as key.
    # This keeps embedding key semantics consistent with runtime lookup.
    embeddings_dict = preprocessed_to_embedding
...
def analyze_script_uniqueness(required_points_df, language='CAN'):
    ...
    for idx, row in required_points_df.iterrows():
        ...
        tokens = set(tokenize_text(preprocess_text(script_text, language=language), language=language, remove_stopwords=True))
...
def process_all_products_batch(excel_file):
    ...
    for sheet_name, product_name, language in product_sheets:
        ...
        try:
            ...
            script_df = apply_composite_point_key_if_configured(script_df, sheet_name)
...
def main():
    ...
    if USE_BATCH_MODE:
        ...
        try:
            ...
            script_df = apply_composite_point_key_if_configured(script_df, DEFAULT_SHEET_NAME)
